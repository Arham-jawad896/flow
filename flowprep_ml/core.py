...
class FlowPrepProcessor:
    ...
    def preprocess(self, file_path: Union[str, Path, pd.DataFrame], **kwargs) -> Dict[str, Any]:
        """
        ...
        """
        try:
            # If DataFrame is provided, skip file validation
            is_dataframe_input = isinstance(file_path, pd.DataFrame)
    ...
    def _load_data(self, file_path: Union[str, Path, pd.DataFrame]) -> pd.DataFrame:
        """Load data from DataFrame or file path with safe, performant defaults."""
        # If a DataFrame is passed directly, use it
        if isinstance(file_path, pd.DataFrame):
            data = file_path.copy()
            self.preprocessing_log.append(f"Loaded DataFrame: {data.shape[0]} rows, {data.shape[1]} columns")
            return data
        ...
        try:
            if file_type == 'csv':
                # Use performant defaults and robust parsing
                data = pd.read_csv(file_path, low_memory=False)
            elif file_type == 'excel':
                # Prefer openpyxl if available
                try:
...
# Main preprocessing function
def preprocess(file_path: Union[str, Path, pd.DataFrame], **kwargs) -> Dict[str, Any]:
    """
